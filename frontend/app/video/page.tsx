...
export default function LiveVideoUpload() {
  ...
  const exportDetections = () => {
    const csv = [
      ...
      ...detections.map(d => `${d.videoTimestamp},${d.plate},${(d.confidence * 100).toFixed(1)}%`)
    ].join('\n')
        ...
        canvas.toBlob((blob) => {
          if (blob && ws.readyState === WebSocket.OPEN) {
            blob.arrayBuffer().then((buffer) => {
              ...
              const combined = new Uint8Array(metadataBytes.length + buffer.byteLength)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-slate-700">
              <h2 className="text-xl font-semibold text-white mb-4">Upload & Process</h2>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-green-600 text-white rounded-lg 
                      hover:bg-green-700 disabled:bg-slate-600 disabled:cursor-not-allowed
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-slate-600 text-white rounded-lg 
                      hover:bg-slate-700 disabled:bg-slate-700 disabled:cursor-not-allowed
                      font-medium transition-colors"
                  >
                    Reset
                  </button>
                  ...
                  {detections.length > 0 && (
                    <button
                      ...
                      className="px-6 py-2.5 bg-purple-600 text-white rounded-lg 
                        hover:bg-purple-700 font-medium transition-colors"
                      title="Export to CSV"
                    >
                      Export CSV
                    </button>
              ...
              {plate && (
                <div className="mt-4 p-4 bg-green-500/10 border border-green-500/30 rounded-lg">
                  <p className="text-green-400 font-semibold text-lg">
                    Current Detection: <span className="font-mono">{plate}</span>
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="bg-slate-800/50 backdrop-blur rounded-xl p-6 border border-slate-700 sticky top-6">
              ...
              <div className="mb-4 p-3 bg-slate-700/50 rounded-lg">
                <p className="text-sm text-slate-300">
                  Total Detections: <span className="font-bold text-cyan-400">{detections.length}</span>
                </p>
              </div>
                  ...
                  detections.map((detection, index) => (
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <button
                          onClick={() => copyToClipboard(detection.plate, index)}
